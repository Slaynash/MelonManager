...
namespace MelonManagerUtils
{
    ...
    class SteamFinder
    {
        ...
        /// <summary>
        /// Retrieves all games and folders by reading the games Steam manifests. The games needs to be marked as installed on Steam.
        /// </summary>
        public List<string> ListGamesPath()
        {
            if (Libraries == null)
                throw new InvalidOperationException("Steam must be found first.");
            ...
            foreach (var library in Libraries)
            {
                string[] manifests = Directory.GetFiles(library, "appmanifest_*.acf");
                foreach (string manifest in manifests)
                {
                    var gameFolderName = ReadInstallDirFromManifest(manifest);
                    if (gameFolderName == null)
                        continue;

                    gamePaths.Add(Path.Combine(library, "common", gameFolderName));
                }
            }
